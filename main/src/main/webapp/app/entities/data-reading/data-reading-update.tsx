...
export const DataReadingUpdate = (props: IDataReadingUpdateProps) => {
  ...
  const { dataReadingEntity, sensors, loading, updating } = props;
        ...
        <Col md="8">
          {loading ? (
            ...
            <AvForm model={isNew ? {} : dataReadingEntity} onSubmit={saveEntity}>
              ...
              <AvGroup>
                <Label for="data-reading-sensor">Sensor</Label>
                <AvInput id="data-reading-sensor" type="select" className="form-control" name="sensor.id">
                  <option value="" key="0" />
                  {sensors
                    ? sensors.map(otherEntity => (
                        <option value={otherEntity.id} key={otherEntity.id}>
                          {otherEntity.sensorNo}
                        </option>
                      ))
                    : null}
                </AvInput>
